...
WebRTC（摄像头、麦克风、`crypto.subtle` 等）只能在**安全上下文**下工作：HTTPS，或者 localhost。当时访问的是 `http://10.10.10.10:10081`——既不是 HTTPS 也不是 localhost，Firefox 和 Safari 严格遵守规范，直接禁用。

Chrome/Edge（Chromium 系）则有一个刻意的放宽：**把非公网 IP（10.x、192.168.x 这类 RFC1918 地址）也判定为可信任来源**——理由是内网 IP 无法被公网中间人攻击。

| 浏览器 | `http://10.10.10.10` 的判定 | Talk 表现 |
| --- | --- | --- |
| Chrome / Edge | 视为可信任（Chromium 放宽） | ✅ 正常 |
| Firefox / Safari | 非安全上下文（严格） | ❌ 无法显示 |
...
- **cloudflared 自带 Prometheus 指标端点**（默认 `127.0.0.1:20241`），连接数、请求量、错误数一应俱全——之前讨论过的"watchdog 盲区"（隧道活着但后端全挂）从此变成了可视化的 5xx 曲线。
- **全部二进制 + systemd 部署**。Prometheus 88MB、cAdvisor 37MB、node_exporter 15MB，整套监控 ~170MB，比一个 jellyfin 还轻。
- **Grafana 数据源用 provisioning 文件注入**（`/etc/grafana/provisioning/`），不用登录 UI 配置，重启自动加载。
...
- **Dashdot**（与 Grafana 功能重叠）：容器 + 镜像（212MB）删除，Glance/UptimeKuma 同步清理；
